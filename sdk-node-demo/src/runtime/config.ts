...
const defaultConfig: DemoConfig = {
  ...
  storageDir: '.ocash-demo',
  cacheDir: '.ocash-demo/cache',
  chains: [
    {
      chainId: 11155111,
...
const defaultAssetsOverride: AssetsOverride = {
...
export async function loadDemoConfig(options?: { configPath?: string | boolean }) {
  const configPath = typeof options?.configPath === 'string' ? options.configPath : 'ocash.config.json';
